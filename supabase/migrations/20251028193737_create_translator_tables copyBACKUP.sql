...
-- ========================================
-- 2. TABLE: terminology (AUTO-LEARNING + TMX READY)
-- ========================================
...
-- TMX Import specific indexes
CREATE INDEX IF NOT EXISTS idx_terminology_import_source ON terminology(imported_from);
CREATE INDEX IF NOT EXISTS idx_terminology_category ON terminology(category);
CREATE INDEX IF NOT EXISTS idx_terminology_imported_at ON terminology(imported_at);
...
-- ========================================
-- 9. UPDATE TRIGGERS FOR UPDATED_AT TIMESTAMPS
-- ========================================
...
DO $$
BEGIN
    IF NOT EXISTS (SELECT 1 FROM pg_trigger WHERE tgname = 'update_translation_memory_updated_at') THEN
        CREATE TRIGGER update_translation_memory_updated_at BEFORE UPDATE ON translation_memory FOR EACH ROW EXECUTE FUNCTION update_updated_at_column();
    END IF;
    
    IF NOT EXISTS (SELECT 1 FROM pg_trigger WHERE tgname = 'update_terminology_updated_at') THEN
        CREATE TRIGGER update_terminology_updated_at BEFORE UPDATE ON terminology FOR EACH ROW EXECUTE FUNCTION update_updated_at_column();
    END IF;
    
    IF NOT EXISTS (SELECT 1 FROM pg_trigger WHERE tgname = 'update_formatting_rules_updated_at') THEN
        CREATE TRIGGER update_formatting_rules_updated_at BEFORE UPDATE ON formatting_rules FOR EACH ROW EXECUTE FUNCTION update_updated_at_column();
    END IF;
    
    IF NOT EXISTS (SELECT 1 FROM pg_trigger WHERE tgname = 'update_translation_projects_updated_at') THEN
        CREATE TRIGGER update_translation_projects_updated_at BEFORE UPDATE ON translation_projects FOR EACH ROW EXECUTE FUNCTION update_updated_at_column();
    END IF;
END $$;